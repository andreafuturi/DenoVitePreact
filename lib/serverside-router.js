...
export function createPageHandler(basePath = "", render, App, isDev = false, handleError) {
  return async function handlePageRoute(path, body, req) {
    ...
    try {
      const html = render(App({ children: Page({ originRequest: req, ...body }) }));

      // 🎨 Helper function to process resources
      const processResources = async (ext, tag) => {
        const resources = globalThis.importedResources.filter(r => r.endsWith(ext));
        const contents = await Promise.all(resources.map(r => Deno.readTextFile(`${Deno.cwd()}/client/${r}`)));
        return `<${tag}>${contents.join("")}</${tag}>`;
      };

      // 📜 Process both scripts and styles
      const [finalScripts, finalStyles] = await Promise.all([processResources(".js", "script"), processResources(".css", "style")]);

      return new Response((isDev ? `<!DOCTYPE html><script>globalThis.dev=true</script>` : "<!DOCTYPE html>") + finalStyles + finalScripts + html, {
