...
export function createPageHandler(basePath = "", render, App, isDev = false, handleError) {
  return async function handlePageRoute(path, body, req) {
    ...
    let Page;
    try {
      // Try direct path first (e.g., about.jsx)
      Page = (await import(`${basePath}/${path || "home"}.jsx`)).default;
    } catch (e) {
      // If not found, try nested path (e.g., about/about.jsx)
      Page = (await import(`${basePath}/${path}/${path}.jsx`)).default;
    }
